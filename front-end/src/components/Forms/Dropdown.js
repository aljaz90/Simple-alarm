...
export const Dropdown = props => {
    ...
    if (props.animationsDisabled) {
        return (
            <OutsideClick className={props.wrapperClassName} onOutsideClick={() => { setQuery(""); setIsDropdownOpen(false); }}>
                    ...
                    <div className={`dropdown--options dropdown--options-${position}`}>
                        {
                            ...
                            <React.Fragment>
                                {
                                    props.searchEnabled &&
                                        <input onChange={e => { setQuery(e.target.value); }} autoFocus={true} placeholder="Seach" className={`dropdown--options--search dropdown--options--search-${props.type}`} />
                                }
                                {
                                    options.map(option => 
                                        (<div onClick={() => { setIsDropdownOpen(false); setSelectedOption(option.key ? option.key : option); props.onSelect(option.key ? option.key : option); }} className={`dropdown--options--item dropdown--options--item-${props.type}`}>
    ...
    return (
        <OutsideClick className={props.wrapperClassName} onOutsideClick={() => {setQuery(""); setIsAnimationDisabled(false); setIsDropdownOpen(false);}}>
            <div className={`dropdown dropdown-${props.type} ${props.wrapperClass}`}>
                ...
                <EzAnimate disabled={isAnimationDisabled} transitionName={`animation--dropdown--${position}`}>
                {
                    isDropdownOpen &&
                        <div className={`dropdown--options dropdown--options-${position}`}>
                            {
                            ...
                            {
                                options.map(option => 
                                    (<div key={option.key ? option.key : option} onClick={() => {setIsAnimationDisabled(false); setIsDropdownOpen(false); setSelectedOption(option.key ? option.key : option); props.onSelect(option.key ? option.key : option);}} className={`dropdown--options--item dropdown--options--item-${props.type}`}>
                                        {option.icon && <IonIcon className={`dropdown--options--item--icon`} name={option.icon} />} {option.text ? option.text : option}
                                    </div>)
                                )
                            }
